...
public class DoorRequestProfile : Profile
{
    public DoorRequestProfile()
    {
        ...
        CreateMap<DoorOpeningRequest, DoorRequestDto>()
            .ForMember(dest => dest.RequestDate, opt => opt.MapFrom(src => src.RequestDate))
            .ForMember(dest => dest.SlotId, opt => opt.MapFrom(src => src.SlotId))
            .ForMember(dest => dest.SlotLabel, opt => opt.MapFrom(src => src.Slot != null ? src.Slot.Label : string.Empty))
            .ForMember(dest => dest.SlotStartTime, opt => opt.MapFrom(src => src.Slot != null ? src.Slot.StartTime : default))
            .ForMember(dest => dest.SlotEndTime, opt => opt.MapFrom(src => src.Slot != null ? src.Slot.EndTime : default))
